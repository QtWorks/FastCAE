...
namespace GUI
{
	SignalHandler::SignalHandler(MainWindow *mainwindow) : _mainWindow(mainwindow)
	{
		...
		connect(mainwindow->getUi()->actionOpenPostFile, SIGNAL(triggered()), this, SLOT(openPostFile()));
		...
		connect(mainwindow->getUi()->actionCreateVector, SIGNAL(triggered()), this, SLOT(createPostVector()));
		connect(mainwindow->getUi()->actionCreateClip, SIGNAL(triggered()), this, SLOT(createPostClip()));
		connect(mainwindow->getUi()->actionCreateSlice, SIGNAL(triggered()), this, SLOT(createPostSlice()));
		connect(mainwindow->getUi()->actionCreateStreamLine, SIGNAL(triggered()), this, SLOT(createPostStreamLine()));
		connect(mainwindow->getUi()->actionCreateISOSurface, SIGNAL(triggered()), this, SLOT(createPostISOSurface()));
		connect(mainwindow->getUi()->actionCreateISOCurve, SIGNAL(triggered()), this, SLOT(createPostISOCurve()));
		connect(mainwindow->getUi()->actionCreateCalculator, SIGNAL(triggered()), this, SLOT(createPostCalculator()));
		connect(mainwindow->getUi()->actionCreateReflection, SIGNAL(triggered()), this, SLOT(createPostReflection()));
		connect(mainwindow->getUi()->actionSaveImage, SIGNAL(triggered()), this, SLOT(savePostPicture()));
		connect(mainwindow->getUi()->actionSaveVideo, SIGNAL(triggered()), this, SLOT(savePostAnimation()));

		connect(mainwindow->getUi()->actionGeoMeshRotate, SIGNAL(triggered()), this, SLOT(GeoMeshRotate()));
	...
	void SignalHandler::openPostFile()
	{
		...
		QString pycode = QString("PostProcess.openPostFile(%1,'%2','%3')").arg(id).arg(type).arg(files);
		Py::PythonAgent::getInstance()->submit(pycode);
		//		pWindow->openFile(files);
		// 		QString pycode = QString("MainWindow.openPostFile(%1,%2,%3)").arg(id).arg(type).arg(files);
		// 		Py::PythonAgent::getInstance()->submit(pycode);
	}
	...
	void SignalHandler::openPlotFile()
	{
		...
		QString files = QFileDialog::getOpenFileName(_mainWindow, tr("open file"), " ", tr("dat(*.dat);;Allfile(*.*)"));
		if (files.isEmpty())
			return;
	...
	void SignalHandler::createPostVector()
	{
		...
		Post::CreateVectorDialog *dlg = new Post::CreateVectorDialog(_mainWindow->getControlPanel()->getPostTreeWidget(), _mainWindow);
		dlg->show();
		// pWindow->setDisplayMode(type);
	}
	...
	void SignalHandler::createPostClip()
	{
		...
		Post::CreateClipDialog *dlg = new Post::CreateClipDialog(_mainWindow->getControlPanel()->getPostTreeWidget(), true, _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::createPostSlice()
	{
		...
		Post::CreateClipDialog *dlg = new Post::CreateClipDialog(_mainWindow->getControlPanel()->getPostTreeWidget(), false, _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::createPostStreamLine()
	{
		...
		Post::CreateStreamLineDialog *dlg = new Post::CreateStreamLineDialog(_mainWindow->getControlPanel()->getPostTreeWidget(), _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::createPostISOSurface()
	{
		...
		Post::CreateISODialog *dlg = new Post::CreateISODialog(_mainWindow->getControlPanel()->getPostTreeWidget(), true, _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::createPostISOCurve()
	{
		...
		Post::CreateISODialog *dlg = new Post::CreateISODialog(_mainWindow->getControlPanel()->getPostTreeWidget(), false, _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::createPostCalculator()
	{
		...
		Post::CreateCalculateDialog *dlg = new Post::CreateCalculateDialog(_mainWindow->getControlPanel()->getPostTreeWidget(), _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::createPostReflection()
	{
		...
		Post::CreateReflectionDialog *dlg = new Post::CreateReflectionDialog(_mainWindow->getControlPanel()->getPostTreeWidget(), _mainWindow);
		dlg->show();
	}
	...
	void SignalHandler::savePostPicture()
	{
		...
		QString fileName = QFileDialog::getSaveFileName(_mainWindow, tr("Select File."), qApp->applicationDirPath(),
														"PNG files(*.png);;BMP files(*.bmp);;JPG files(*.jpg)");

		if (fileName.isEmpty())
			return;

		QString pycode = QString("PostProcess.savePostImage(%1,'%2')").arg(gw->getID()).arg(fileName);
		Py::PythonAgent::getInstance()->submit(pycode);
