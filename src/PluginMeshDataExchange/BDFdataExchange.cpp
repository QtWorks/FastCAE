...
namespace MeshData
{
	...
	const QMap<QString, QPair<int, VTKCellType>> allSupportElements = {
		...
		std::map<QString, QPair<int, VTKCellType>>::value_type("CQUAD8", qMakePair(8, VTK_QUADRATIC_QUAD)),
		std::map<QString, QPair<int, VTKCellType>>::value_type("CHEXA", qMakePair(8, VTK_HEXAHEDRON)),
		std::map<QString, QPair<int, VTKCellType>>::value_type("CTETRA", qMakePair(10, VTK_QUADRATIC_TETRA))

	};

	BDFDataExchange::BDFDataExchange(const QString &fileName, MeshOperation operation, GUI::MainWindow *mw, int modelId) : MeshThreadBase(fileName, operation, mw),
																														   _fileName(fileName),
	...
	bool BDFDataExchange::readNodes(vtkUnstructuredGrid *g, QString &line, MeshKernal *k)
	{
		...
		do
		{
			...
			// qDebug() << "Point : index=" << index << "; x=" << point[0] << "; y=" << point[1] << "; z=" << point[2] ;
			// points->InsertPoint(index - 1, point);
